...
2. **ASR inference cost exceeds ~USD 500/month** _and_ AWS Inferentia2 benchmarking shows a sustained >40% cost reduction over Cloud Run L4 for the same model.
...
- **AWS (ECS Fargate or App Runner + RDS + ElastiCache + S3 + CloudFront).** Rejected for _this_ iteration on the Worker-fit, one-shot-job, and solo-ergonomics grounds above. Reconsidered if any of the triggers above fire.
